...
import type { RequestFn, Method } from '../main'
...
export class Tacos {
    ...
    constructor(instanceConfig) {
        ...
        const downs:Method[]= ['downloadFile', "uploadFile"]
        const _this=this
        downs.forEach(key=>{
            this.forEachMethodNoData.apply(_this,[key])
        
        })
        const reqs:Method[]=['post', 'get', 'post', 'put', 'delete']
        reqs.forEach(key=>{
            this.forEachMethodWithData.apply(_this,[key])
        
        } )
    }

    private forEachMethodNoData = (method: Method) => {
        this[method] = function <T> (url, config) {
    ...
    private forEachMethodWithData = (method: Method) => {

        this[method] = function<T> (url, data, config) {
            return this.request<T>({
                ...config,
                ...{
                    method,
    ...
    mergebase(newItem, base = this.defaults) {
        if (base.baseUrl) {
